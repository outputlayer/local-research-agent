"""Тесты lra/plan.py: модель Plan/Task/Revision, persistence, mutations, guard."""
...
def test_save_and_load_roundtrip(plan_env):
    plan, _ = plan_env
    p = plan.reset("x")
    p.add_task("новая подтема", origin="emerged", why="test")
    plan.save(p)
    reloaded = plan.load()
...
def test_close_task_sets_done_and_clears_focus(plan_env):
    ...
    p.close_task(focus_id, evidence=["kb:paper1"], why="готово")
...
def test_split_task_creates_children_and_drops_parent(plan_env):
    plan, _ = plan_env
    p = plan.reset("x")
    tid = p.tasks[1].id  # не трогаем focus
    children = p.split_task(tid, ["под A", "под B", "под C"], why="слишком широко")
...
def test_increment_attempts_and_block(plan_env):
    ...
    for _ in range(plan.MAX_ATTEMPTS_PER_TASK):
        p.increment_attempts(tid, iter_=1, why="пусто")
...
def test_guard_blocks_after_max_attempts(plan_env):
    ...
    for i in range(plan.MAX_ATTEMPTS_PER_TASK):
        ...
        p = plan.load()
    # задача должна быть blocked, фокус — ротирован на следующую open
...
# ── sync_focus_from_md (обратная совместимость с write_plan) ──────────
...
def test_sync_focus_from_md_plan_complete_closes_all_open(plan_env):
    ...
    assert plan.sync_focus_from_md(p, md, iter_=1) is True
    # все задачи должны быть done, плана-corrective с именем «PLAN_COMPLETE» быть не должно
