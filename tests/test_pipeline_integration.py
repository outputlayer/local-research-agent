"""Integration test for research_loop with a mocked LLM.

Replaces pipeline.build_bot and pipeline._run_agent so that instead of real LLM calls
each role deterministically produces the required file artifacts via @register_tool.
This catches pipeline-orchestration regressions without loading MLX.
"""
import pytest


def _patch_paths(monkeypatch, tmp_path):
    """Redirects ALL path refs (config/memory/tools/pipeline/validator/metrics/kb/plan) to tmp_path."""
...
def test_research_loop_end_to_end(tmp_path, monkeypatch):
    ...
    # Bot factory — recognize role by system_message
    ...
    # Deterministic simulation of tool calls by each role
    def fake_run_agent(bot, messages, icon):
        role = bot.role
        if role == "explorer":
            # Pre-register id in kb — otherwise the pre-append verifier
            # (notes_strict) would block writes with unknown arxiv-id.
            ...
            tools.AppendNotes().call(
                {"content": "## [T1] survey architectures\n"
                            "[2401.00001] survey on transformer architectures\n"
                            "[2401.00002] attention analysis in survey architectures\n"
                            "[repo: foo/bar ★1200 Python] https://github.com/foo/bar\n"
                            "  modular pipeline with plugin architecture"})
            tools.AppendLessons().call({"content": "[iter 1] worked: hf+gh; did NOT work: —; next: synth"})
        elif role == "replanner":
            # After the first iteration declare the plan complete → early stop
            tools.WritePlan().call({"content": "# Plan: test\n\n[FOCUS] PLAN_COMPLETE\n\n"
                                                "## [TODO]\n\n## [DONE]\n- test: covered\n"})
        elif role == "synthesizer":
            tools.WriteSynthesis().call({"content":
                "## [BRIDGE]\n[2401.00001] <-> [2401.00002]: bridge\n\n"
                "## [CONTRADICTION]\nconflict between [2401.00001] and [2401.00002]\n\n"
                "## [GAP]\ngap for [2401.00001]\n\n"
                "## [EXTRAPOLATION]\ntrend from [2401.00001] to [2401.00002]\n\n"
                "## [REUSE]\n[repo: foo/bar] — adopt pipeline modularity\n\n"
                "## [TESTABLE]\nhypothesis for [2401.00001]: verify\n"})
        elif role == "writer":
            tools.WriteDraft().call({"content":
                "# Report\n\n## Introduction\nabout transformers [2401.00001] and attention [2401.00002]\n\n"
                "## Novel Insights\n[BRIDGE] [CONTRADICTION] [GAP] [EXTRAPOLATION] [REUSE] [TESTABLE]\n\n"
                "## Practical Implementations\n- [repo: foo/bar] architectural benchmark\n\n"
                "## Sources\n1. [2401.00001]\n2. [2401.00002]\n"})
    ...
    # validator must NOT hit the network — disable external hf papers info calls
    ...
    # metrics.json must appear and contain key fields
    import json
    metrics_file = tmp_path / "metrics.json"
    assert metrics_file.exists(), "pipeline must write metrics.json"
    data = json.loads(metrics_file.read_text(encoding="utf-8"))
    assert data["query"] == "test topic"
    assert data["finished_at"] is not None
    assert len(data["iterations"]) >= 1
    assert data["iterations"][0]["new_arxiv_ids"] >= 1
    assert data["valid_ids"] == 2  # from our validate_draft_ids mock
    # specialized critics = 2 rounds (fact + structure), both APPROVED
    ...
    assert memory_files, "pipeline must save run-summary into research/memory/"


def test_research_loop_early_stop_on_plan_complete(tmp_path, monkeypatch):
    """PLAN_COMPLETE after the 1st iteration → pipeline unfolds to synth/writer."""
    from lra import pipeline, tools
    _patch_paths(monkeypatch, tmp_path)
    ...
    def fake_build_bot(system_message, tool_names, max_tokens=None):
        role = "explorer" if "You are a researcher" in system_message else (
            "replanner" if "You are the REPLANNER" in system_message else "other")
        return StubBot(role)
    ...
    # depth=10, but PLAN_COMPLETE after 1st → explorer must be called at most 1-2 times
