"""Pre-append verifier: blocks writing to notes.md with an unverified arxiv-id."""
import pytest
...
def test_verify_ids_empty_kb(_isolated_research):
    ...
    known, unknown = verify_ids_against_kb("see [2401.00001] and [2402.12345]")
    assert known == set()
...
def test_append_notes_allows_repo_only(_isolated_research):
    """Entries with repo: but without arxiv-id do not require verification."""
    from lra import tools
    result = tools.AppendNotes().call(
        {"content": "[repo: foo/bar ★150 Python] — modular architecture"}
    )
...
def test_append_notes_lenient_mode(_isolated_research, monkeypatch):
    """If notes_strict=False — verifier is disabled."""
    from lra import config, tools
    # Disable strict via monkeypatch on CFG
